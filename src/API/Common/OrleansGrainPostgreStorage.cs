...
{
    public static class OrleansGrainPostgreStorage
    {
        private static string _logPrefix = "POSTGRESQL_ORLEANS_GRAIN_STORAGE - ";
        ...
        public static async Task EnsureGrainStorageTablesExist(string connectionString)
        {
            Log.Information($"{_logPrefix}@ EnsureGrainStorageTablesExist()\nConnectionString: {connectionString}");

            var _OrleansRemindersTableExists = await TableExists("public", "OrleansStorage", connectionString);
...
');";
            ...
            //            var _migration = @"-- Run this migration for upgrading the PostgreSQL persistence table and routines for deployments created before 3.6.0
